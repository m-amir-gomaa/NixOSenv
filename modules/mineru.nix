...
in
{
  ...
  # ── automatic setup on activation (DEPRECATED) ────────────────────────────────
  # DISABLED: Running pip/network-dependent scripts during system activation
  # is prone to hangs and can cause emergency mode if the rebuild fails mid-way.
  # Run `sudo mineru-setup` manually after switching if you need to update.
  #
