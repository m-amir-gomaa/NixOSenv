...
start_recording() {
	...
	if kill -0 "$REC_PID" 2>/dev/null; then
		...
		echo "$MODE" >"$MODEFILE"
		notify-send "⏺️ $MODE_NAME RECORDING" "$OUTPUT" -t 4000
	else
		notify-send "❌ FAILED TO START RECORDING" "Check if wl-screenrec and VAAPI drivers are working."
	fi
}
...
toggle_recording() {
	...
	if [ "$STATE" = "running" ]; then
		...
		notify-send "⏸️ RECORDING PAUSED" "Press key again to resume" -t 1000
	else
		kill -CONT "$PID"
		echo "running" >"$STATEFILE"
		notify-send "▶️ RECORDING RESUMED" "Press key again to pause" -t 2000
	fi
}
...
stop)
	stop_recording
	notify-send "⏹️ ALL RECORDING STOPPED" -t 3000
	;;
toggle)
	toggle_recording "$2"
	;;
